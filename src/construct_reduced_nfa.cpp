...
void concat_procedure(nfa_t &nfa, lmap_t &node_map, nfa_stack_t &stack, amap_t &arc_map, buffer_t &buffer, catsites_t& cats)
{
    ...
    arc_t cat_arc = update_arc_map(nfa, node_map, arc_map, subgraph1.start, subgraph2.end);
...
{
    Subgraph node2 = stack.top();
    stack.pop();
    Subgraph node1 = stack.top();
    stack.pop();
    ...
    update_arc_map(nfa, node_map, arc_map, split_node, node1.start);
    update_arc_map(nfa, node_map, arc_map, split_node, node2.start);

    node_t ghost_node = nfa.addNode();
    node_map[ghost_node] = Ghost;
    update_arc_map(nfa, node_map, arc_map, node1.end, ghost_node);
    update_arc_map(nfa, node_map, arc_map, node2.end, ghost_node);
